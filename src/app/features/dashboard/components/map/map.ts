  ...
  mapUpdater = effect(() => {
    ...
    this.geocodingService.geocode(tour.route.start).then((startCoords) => {
      this.geocodingService.geocode(tour.route.destination).then((destCoords) => {
        ...
        const startMarker = this.L!.marker(startCoords)
          .addTo(this.map!)
          .bindPopup(`Start: ${tour.route.start}`);
        const destMarker = this.L!.marker(destCoords)
          .addTo(this.map!)
          .bindPopup(`Destination: ${tour.route.destination}`);
        ...
        this.routingService
          .getRouteGeometry(startCoords, destCoords, tour.route.travelType as TravelType)
          .then((routeCoords) => {
            if (!this.map || !this.L) return;

            const path = routeCoords ?? [startCoords, destCoords];
            this.currentRouteLine = this.L.polyline(path, { color: '#2563eb', weight: 4 }).addTo(
              this.map,
            );
